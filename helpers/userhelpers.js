...
let userHelper = {
    ...
    addToCart: (userId, proId) => {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (cart) {
                ...
                if (cartProduct) {
                    cartModel.updateOne({ user: userId, 'cartItems.product': proId }, { $inc: { 'cartItems.$.quantity': 1 } }).then((data) => {
                        response.inc = true
                        resolve(response)
                    })
                } else {
    ...
    addToWishList: (userId, proId) => {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                let cartItem = await cartModel.findOne({ user: userId, 'cartItems.product': proId })
                if (cartItem) {
                    response.cart = true
                    resolve(response)
                } else {
                    if (userWishList) {
                        let exist = await wishListModel.findOne({ user: userId, 'wishListItems.product': proId })
                        if (!exist) {
                            let conditions = {
                                user: userId,
    ...
    editProfile: (userData) => {
        return new Promise(async (resolve, reject) => {
            ...
            if (user2.email == user1.email || !user2) {
                userModel.findByIdAndUpdate(id, { name: name, email: email, alternativeNumber: alternativeNumber }).then((data) => {
    ...
    applyCoupon: (code, id) => {
        return new Promise(async (resolve, reject) => {
            ...
            if (coupon) {
                let couponUser = await couponModel.findOne({ code: code.code, users: { '$in': [id] } })
                ...
                userHelper.cartTotal(cartProducts.cart).then((response) => {
                    ...
                    order.save().then(async (data) => {
                        ...
                        for (let i = 0; i < cartItems.length; i++) {
                            productModel.findByIdAndUpdate(cartItems[i].product, { $inc: { quantity: -cartItems[i].quantity } }).then((data) => {
                                console.log(data);
                            })
                        }
                        resolve(data);
    ...
    couponUser: (userId, coupon) => {
        return new Promise((resolve, reject) => {
            console.log(coupon)
            couponModel.findByIdAndUpdate(coupon._id, { $push: { 'users': userId } }).then((data) => {
                resolve()
            })
        })
    },
    changeOrderStatus: (id, userId) => {
        console.log(id)
        return new Promise(async (resolve, reject) => {
            orderModel.findOneAndUpdate({ _id: id }, { orderStatus: true, deliveryStatus: 'processing' }).then((data) => {
    ...
    getAllProducts: () => {
        return new Promise(async (resolve, reject) => {
            productModel.find({}).populate('categories').lean().then((products) => {
                resolve(products);
            })
        })
    },
