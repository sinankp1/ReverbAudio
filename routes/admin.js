...
router.post("/update-product", isLogin, (req, res) => {
  adminHelper.updateProduct(req.body).then((response) => {
    req.files.image1.mv(
      "public/images/product-image/" + req.body.id + "1.jpg",
      (err, done) => {
        if (err) {
          console.log(err);
        }
    ...
    req.files.image2.mv(
      "public/images/product-image/" + req.body.id + "2.jpg",
      (err, done) => {
        if (err) {
          console.log(err);
        }
    ...
    req.files.image3.mv(
      "public/images/product-image/" + req.body.id + "3.jpg",
      (err, done) => {
        if (err) {
          console.log(err);
        }
...
router.post("/add-banner", isLogin, (req, res) => {
  adminHelper.addBanner(req.body).then((response) => {
    if (response.exist) {
      ...
      req.files.image.mv(
        "public/images/banner-image/" + response.data._id + ".jpg",
        (err, done) => {
          if (err) {
            console.log(err);
          }
...
router.post("/edit-banner", isLogin, (req, res) => {
  ...
  adminHelper.editBanner(req.body).then((response) => {
    req.files.image.mv(
      "public/images/banner-image/" + req.body.id + ".jpg",
      (err, done) => {
        if (err) {
          console.log(err);
        }
