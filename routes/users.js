...
router.get('/product-details/:id', async (req, res, next) => {
  ...
  try {
    ...
    adminHelpers.getAllCategory().then((cats) => {
      res.render('user/product-details', { session, cats, product, user: true });
    })
  } catch (err) {
    next(err);
  }
...
router.post('/quantityPlus/:id', isLogin, (req, res, next) => {
  try {
    userhelpers.quantityPlus(req.params.id, req.session.currentuser._id).then((response) => {
      res.json({ response })
    })
  } catch (err) {
    next(err)
...
router.post('/quantityMinus/:id', isLogin, (req, res) => {
  try {
    userhelpers.quantityMinus(req.params.id, req.session.currentuser._id).then((response) => {
      res.json({ response })
    })
  } catch (err) {
    next(err)
...
router.post('/deleteFromCart/:id', isLogin, (req, res, next) => {
  try {
    userhelpers.deleteFromCart(req.session.currentuser._id, req.params.id).then((response) => {
      res.json({ response })
    })
  } catch (err) {
    next(err)
...
router.post('/applyCoupon', isLogin, (req, res) => {
  userhelpers.applyCoupon(req.body, req.session.currentuser._id).then((response) => {
    if (response.status) {
...
router.post('/addToWishList/:id', isLogin, (req, res, next) => {
  userhelpers.addToWishList(req.session.currentuser._id, req.params.id).then((response) => {
...
router.get('/checkWishlist/:id', isLogin, (req, res, next) => {
  ...
  userhelpers.checkWishlist(req.session.currentuser._id, req.params.id).then((wishList) => {
    res.json({ wishList })
  }).catch((err) => {
    next(err)
  })
...
router.post('/removeWishListItem/:id', isLogin, (req, res, next) => {
  userhelpers.removeWishListItem(req.session.currentuser._id, req.params.id).then((response) => {
    res.json({ response })
  }).catch((err) => {
    next(err)
  })
...
router.get('/userprofile', isLogin, (req, res) => {
  userhelpers.getUser(req.session.currentuser._id).then((response) => {
    ...
    res.render('user/user-profile', { emailExist, userDetails, session, user: true })
  })
...
router.post('/placeOrder', isLogin, (req, res) => {
  ...
  userhelpers.PlaceOrder(orderDetails, userId).then(async (order) => {
    ...
    if (order.paymentDetails === 'COD') {
      if (req.session.coupon) {
        await userhelpers.couponUser(req.session.currentuser._id, req.session.coupon)
      }
      userhelpers.changeOrderStatus(order._id, req.session.currentuser._id).then((data) => {
...
router.post('/verifyPayment', isLogin, (req, res) => {
  ...
  userhelpers.verifyPayment(req.body).then(async () => {
    ...
    userhelpers.changeOrderStatus(req.body.order.receipt, req.session.currentuser._id).then(() => {
...
router.post('/editProfile', isLogin, (req, res) => {
  userhelpers.editProfile(req.body).then((response) => {
    ...
    if (response.emailexist) {
      ...
      req.files.image1.mv('public/images/user-images/' + req.body.id + '.jpg', (err, done) => {
